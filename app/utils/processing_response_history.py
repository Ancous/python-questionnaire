...
def process_user_answer(
    ...
    session["number_questions_answered"] = (
        NUMBER_OF_QUESTIONS
        - AnsweredQuestions.get_numbers_count(se, user_id=session["user_id"])
    )
    UserStatistic.set_answer_for_user_and_question(
        se, user_id, question_id, answer_option.id
    )
